...
from feature_second import extract_feature
import numbers
from speed_predictor import *
import network
from bdda import BDDA
from network import FC
from Json_to_speed import get_interpolated_speed
from sklearn.metrics import f1_score,precision_score,recall_score, roc_curve, roc_auc_score
from img_overlay import overlay
...
def main():
    ...
    optimizer_speed = torch.optim.Adam(speed_model.parameters(),lr=args.lr)
    # if args.resume:
    ...
    if not args.no_train:
        ...
        speed_array, speed_idx = speed_reader('BDDA/training/gps_jsons')
    ...
    if not args.no_train:
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            # train for one epoch
            train(train_loader, model, speed_model, criterion, optimizer, optimizer_speed, epoch, args, speed_array, speed_idx)
...
def train(train_loader, model, speed_model, criterion, optimizer, optimizer_speed, epoch, args, speed_array, speed_idx):
    ...
    for i, (input, target, img_name) in enumerate(train_loader):
        ...
        print(input.shape)
        output = model(input)
        loss_attention = criterion(output, target)
        losses.update(loss_attention.item(), input.size(0))
        ...
        for j in range(len(input)):
            # print(len(input))
            heatmap_img = heatmap[j]
            heatmap_s = torchvision.transforms.functional.to_pil_image(heatmap_img.cpu())
            small_feature = overlay(heatmap_s,img_name[j])
            extracted_feature = extract_feature(small_feature)
            print(extracted_feature.shape)

            if int(img_name[j].split('_')[1]) % 333 ==0:
                nu = int(img_name[j].split('_')[1])/333
            else:
                nu = math.ceil(int(img_name[j].split('_')[1])/333)
            idxx = str(img_name[j].split('_')[0])+'_'+str(int(nu))

            # print(speed_idx)
            # idxxx = speed_idx.index(idxx)
            if idxx in speed_idx:
                target_speed = torch.tensor(speed_array[speed_idx.index(idxx)]).cuda()

                speed_output = speed_model(output.flatten())

                loss_speed = abs(torch.sqrt(torch.square(speed_output[0]) + torch.square(speed_output[1]))-torch.sqrt(torch.square(target_speed[0]) + torch.square(target_speed[1])))
